...
const REG = uniq(D.flatMap(p => p.reg));
...
const VW = {
  ...
  regions: {t:'Ургах бүс нутаг',items:D,ft:'Бүс нутаг, ургах орчны шүүлтүүрийг тархацын эх бичвэрийн түлхүүр үгээр гаргасан. Бүрэн тархацыг ургамлын дэлгэрэнгүй мэдээллээс үзнэ.'}
};
$('#plant-total').textContent = D.length;
$$('[data-n]').forEach(e => { const k=e.dataset.n; e.textContent=k==='regions' ? `${REG.length} бүс нутаг · ${D.filter(p=>p.reg.length).length} ургамал` : `${VW[k].items.length} ургамал`; });
...
function show(v) {
  ...
  $('#regs').innerHTML='<button class="on" data-r="">Бүгд</button>'+REG.map(r=>`<button data-r="${esc(r)}">${esc(r)} (${D.filter(p=>p.reg.includes(r)).length})</button>`).join('');
  scrollTo(0,0);draw();
}
...
function draw() {
  ...
  const found=VW[view].items.filter(p=>(!q||`${p.no} ${p.mn} ${p.laFull} ${p.fam} ${p.famMN}`.toLowerCase().includes(q))&&Object.entries(filters).every(([k,v])=>!v||p[k]===v)&&(!hab||p.hab.includes(hab))&&(!reg||p.reg.includes(reg)));
  ...
  $('#grid').innerHTML=found.length?found.map(p=>{
    ...
    const sub=view==='regions'?(p.reg.join(', ')||'Бүс нутаг заагаагүй'):view==='redlist'?`Олон улс: ${iu(p.iw)}`:`№${p.no} · ${p.fam}`;
